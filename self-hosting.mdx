...
By default, Sure limits NDJSON file uploads (used by the Sure import format) to a fixed size. Self-hosted admins can raise this limit with:

```txt
SURE_IMPORT_MAX_NDJSON_SIZE_MB=50
```

Set the value to the maximum file size in megabytes you want to allow. This variable is now respected by both the GUI upload path and the API upload path.

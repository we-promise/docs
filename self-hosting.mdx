...
4. Select an agent. After you save a valid URL and token, Sure calls `/v1/models` on the endpoint and populates an agent dropdown in the UI. Select the agent you want to use and save. The external service manages the model behind each agent; Sure only handles routing.

   To skip the UI and pin an agent via environment variable, set:

   ```txt
   EXTERNAL_ASSISTANT_MODEL=openclaw/my-agent
   ```

   When `EXTERNAL_ASSISTANT_MODEL` is set, the dropdown in the UI is disabled and the environment value is always used.
...
See the `docs/hosting/ai.md` file in the Sure repository for full configuration details including agent model selection, session keys, and email allowlisting.

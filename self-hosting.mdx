...
By default, Sure NDJSON imports uploaded through the GUI are limited to a fixed maximum size. Self-hosted admins can raise this limit with:

```txt
SURE_IMPORT_MAX_NDJSON_SIZE_MB=50
```

Set the value to the maximum file size in megabytes you want to allow. This limit applies to both the GUI upload path and the API upload path, so raising it here affects all import entry points consistently.

...
By default, Sure NDJSON imports are limited to a fixed size. Self-hosted admins can raise this limit with:

```txt
SURE_IMPORT_MAX_NDJSON_SIZE_MB=200
```

This limit applies to all Sure NDJSON upload paths, including the GUI upload and the API. If you raise this value, make sure your web server and reverse proxy are also configured to accept larger request bodies.

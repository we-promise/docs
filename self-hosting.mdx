...
Self-hosted admins can raise the maximum allowed size for Sure NDJSON imports by setting:

```txt
SURE_IMPORT_MAX_NDJSON_SIZE_MB=200
```

This limit applies to both the GUI upload (Settings → Imports) and the API upload paths. If you previously set this variable and found the GUI still rejected large files, update to the latest version — the GUI now correctly reads this setting.

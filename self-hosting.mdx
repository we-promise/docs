...
Self-hosted instances include an admin system health page at `/settings/admin/system_health`. This page is only accessible to super admins.

The page shows live Sidekiq state:
...
When the Sidekiq worker is not processing jobs, a banner appears on every page for super admins. The banner explains that background data may be stale and links directly to the system health page.
...
To start the worker if it is not running:
...
The detection thresholds can be adjusted with environment variables:

| Variable | Default | Description |
| --- | --- | --- |
| `SIDEKIQ_HEALTH_HEARTBEAT_TIMEOUT` | `120` | Seconds before a missing heartbeat is considered stale |
| `SIDEKIQ_HEALTH_LATENCY_THRESHOLD` | `300` | Seconds of queue latency before the instance is considered unhealthy |
| `SIDEKIQ_HEALTH_CACHE_TTL` | `60` | Seconds to cache the health snapshot between requests |

The system health page always bypasses the cache so you see fresh state after restarting the worker.

...
By default, Sure NDJSON imports (full data exports) are limited to a fixed size. Self-hosted admins can raise this limit with:

```txt
SURE_IMPORT_MAX_NDJSON_SIZE_MB=200
```

Set the value to the maximum file size in megabytes you want to allow. This limit applies to both the GUI upload and the API upload paths.

...
By default, Sure NDJSON imports are limited to a fixed maximum file size. Self-hosted admins can raise this limit using an environment variable:

```txt
SURE_IMPORT_MAX_NDJSON_SIZE_MB=200
```

This setting applies to all Sure import upload paths, including the GUI and the API. Without it, the GUI upload enforces the built-in default regardless of what the API allows.

...
<Steps>
  ...
  <Step title="Configure redirect URIs">
    Add your callback URL. Use the callback URL shown for the provider in Sure; for this example:
    ```
...
- `https://app.example.com/auth/authentik/callback`
...
### Groups claim request

When you include `groups` in the scopes or configure [role mapping](/authentication/role-mapping), the application automatically sends an explicit OIDC `claims` parameter on the authorization request, asking the provider to return `groups` as an essential claim in both the ID token and userinfo response:
...
Some providers (such as Authelia) do not return the `groups` claim based on scope alone. This explicit request ensures the claim is included so role mapping can take effect. No additional configuration is required to enable this behavior.
...
- Copy the callback URL from `/admin/sso_providers` if you are unsure; older examples using `/auth/openid_connect/callback` only apply when the provider ID is `openid_connect`
...
**Solution**:
- Add `groups` scope to your configuration, or configure [role mapping](/authentication/role-mapping) (either triggers an explicit `claims` request for `groups`)
- Verify your identity provider includes groups in the token
- Check your group-to-role mapping configuration
- Some providers require additional configuration to release the `groups` claim, even when it is requested as essential

### Account deactivated during SSO linking

**Problem**: A user's account is deactivated at the same moment they are completing an SSO account-link flow. The link is rejected, but the user wants to retry once reactivated.

**Solution**:
- The linking flow rolls back atomically: no identity record or audit entry is created for the rejected request, and the one-time linking code is restored.
- Once a super admin reactivates the user from `/admin/users`, the user can restart the link from their identity provider without any manual cleanup.

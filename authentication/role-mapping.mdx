...
When either the `groups` scope is requested or a role mapping is configured, the application automatically sends an explicit OIDC `claims` parameter asking the provider to return `groups` as an essential claim in both the ID token and userinfo response. This helps with providers (for example, Authelia) that do not release the `groups` claim from scope alone.

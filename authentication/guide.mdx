...
<Steps>
  <Step title="Create OAuth client in your IdP">
    Configure the redirect URI shown for the provider in Sure. For a provider with the ID `authentik`, use:
    ```
    https://yourdomain.com/auth/authentik/callback
    ```
  </Step>
...
Each provider must have a unique `name` field, which determines the callback URL path. For example, a provider named `authentik` uses `https://your-domain.com/auth/authentik/callback`.
...
When a JIT login creates a new family, the user who creates it is assigned the `admin` role for that family regardless of the default role setting. Super admin accounts retain their elevated defaults. This ensures the first user of a new family always has the access needed to configure it.
...
**Safety guard for disconnecting:** Sure prevents a user from disconnecting their last SSO provider when they have no local password set. The disconnect button is hidden in that case, and a warning is shown prompting the user to set a password first. If the user has either a second SSO provider connected or a local password, disconnecting proceeds and the event is recorded in the SSO audit log as an `unlink` event.

### User actions

**Deactivate user**: Immediately cut off all access without removing data. All web sessions are destroyed, all OAuth tokens and API keys are revoked, and every subsequent login attempt is rejected with HTTP 401.
```
/admin/users → Actions popover → Deactivate User
```

**Reactivate user**: Restore access. The user must log in again; existing sessions and tokens are not restored.
```
/admin/users → Actions popover → Reactivate User
```

**Delete user**: Permanently remove (cannot be undone)
```
/admin/users → Actions popover → Delete User → Type email to confirm
```

**Sync roles**: Force role synchronization from IdP
```
/admin/users → Actions popover → Sync Roles from SSO
```
...
- Authentik: `https://yourdomain.com/auth/authentik/callback`
- Google Workspace: `https://yourdomain.com/auth/google/callback`

The callback URL is shown in admin UI when editing a provider (with copy button). Use that exact value in your identity provider. Older examples that use `/auth/openid_connect/callback` only apply to a legacy provider whose name is literally `openid_connect`.
...
- Client secrets encrypted at rest using Rails 7.2 ActiveRecord Encryption
- In self-hosted mode, encryption keys are derived from `SECRET_KEY_BASE` unless you set dedicated `ACTIVE_RECORD_ENCRYPTION_*` keys
- Keep the same `SECRET_KEY_BASE` for the lifetime of an existing instance. Changing it changes the derived encryption keys and can make existing encrypted fields, including user email lookups, unreadable.
...
- If users suddenly see "invalid username or password" after encryption changes, restore the previous `SECRET_KEY_BASE` or previous `ACTIVE_RECORD_ENCRYPTION_*` values first, then rerun the backfill task if you intentionally moved to dedicated keys.
...
Sure uses Rack::Attack to throttle credential-checking endpoints in production and staging. Rate limiting is disabled in development and test environments to avoid flakiness.

Every endpoint that checks a password, TOTP code, or backup code is throttled by **both** IP address and a credential-identifying discriminator (email address, or the session-bound user ID for MFA step-up). Both throttles must pass — an attacker cannot bypass one by rotating IPs against a single target or by spraying many addresses from one IP.

| Endpoint | Throttle keys | Limit |
| --- | --- | --- |
| `POST /sessions` — web login | IP + email | 10 req/min each |
| `POST /mfa/verify` — TOTP and backup codes | IP + session user ID | 10 req/min each |
| `POST /password_reset` — password reset | IP + email | 10 req/min each |
| `POST /oidc_account/create_link` — SSO identity linking | IP + email | 10 req/min each |
| `POST /api/v1/auth/login` — mobile/API login | IP + email | 10 req/min each |
| `POST /api/v1/auth/sso_link` — mobile SSO identity linking | IP + email | 10 req/min each |
| `POST /oauth/token` — OAuth token endpoint | IP | 10 req/min |
| `POST /register` — OAuth registration | IP | 10 req/min |
| `POST /mfa/webauthn_options`, `/mfa/verify_webauthn`, `/sessions/passkey` — WebAuthn MFA | IP | 10 req/min |
| `POST /sessions/passkey_options` — passkey challenge (conditional mediation) | IP | 30 req/min |
| Admin endpoints (`/admin/*`) | IP | 10 req/min |

Requests that exceed a limit receive HTTP 429. The throttle also matches format-suffixed paths (e.g. `/sessions.json`) so scripted clients cannot bypass it by appending a file extension.

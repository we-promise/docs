...
Sure performs safe user removal: active sessions are invalidated, any pending token issuance for the account is rejected, and the deletion fails closed if session cleanup cannot complete — preventing a partially-removed account from remaining accessible.
...
<Note>
If a deleted user's session token is presented after removal, Sure rejects it at the session creation step rather than allowing access. Inactive users are also blocked from receiving new tokens.
</Note>

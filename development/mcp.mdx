...
These tools are available to all MCP clients and to the built-in AI assistant.

| Tool | Description |
| --- | --- |
| `get_transactions` | Search transactions with optional filters (accounts, categories, merchants, tags, types, statuses), sorting, and pagination |
| `get_accounts` | List accounts with current balances, account metadata, and optional historical balance series |
| `get_merchants` | List merchants with IDs and names; use IDs with `update_transaction` |
...
| `get_balance_sheet` | Retrieve balance-sheet and net-worth data with configurable period, date range, and interval |
...
| `get_recurring_transactions` | List detected and manual recurring transactions with status filter and optional upcoming-bill window |
...
Preview tools are available when the corresponding feature is enabled on your Sure instance. They do not appear on the default MCP surface.

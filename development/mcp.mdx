...
| Tool | Description |
| --- | --- |
| `get_transactions` | Search transactions with filters (account, category, merchant, tag, type, status) and pagination; sort by date or amount |
...
Preview tools are only available when the corresponding feature is enabled for the family.

...
| Tool | Description |
| --- | --- |
| `get_transactions` | Search transactions with optional filters (account IDs, category names, merchant names, tag names, types, statuses) and pagination |
...
| `get_merchants` | List merchants with IDs, exact names, and source; use IDs with `update_transaction` |
...
| Tool | Description |
| --- | --- |
| `get_insights` | Return the visible insights feed (type filter, acknowledged toggle, limit) without marking items read |
| `get_valuations` | List valuation entries newest-first with kind and citation notes, scoped to accessible visible accounts |
...
| `record_valuation` | Record a valuation entry for an account with an optional citation note |

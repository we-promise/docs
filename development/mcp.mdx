...
| Tool | Description |
| --- | --- |
| `get_transactions` | Search transactions with filters (exact names or ids), sorting by date or absolute amount, and pagination; also accepts `types` (income/expense/transfer) and `statuses` (pending/confirmed) |
| `get_recurring_transactions` | Detected and manual recurring transactions (subscriptions, bills, salaries) with expected dates and per-currency totals of active non-transfer items |

...
| Tool | Description |
| --- | --- |
| `get_transactions` | Search transactions with optional filters (account IDs, categories, merchants, tags, types, statuses) and pagination |
...
| `get_recurring_transactions` | List detected and manual recurring transactions with per-currency totals; filter by status and upcoming window |
...
These tools are available when the corresponding feature is enabled on your Sure instance.

| Tool | Description |
| --- | --- |
| `get_insights` | Return the visible insights feed; supports type filter, acknowledged toggle, and limit |

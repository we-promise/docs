...
| Tool | Description |
| --- | --- |
| `get_transactions` | Search transactions with optional filters (accounts, categories, merchants, tags, date range, amount range, type, status) and pagination |
...
| `get_merchants` | List merchants visible to the user, including IDs and names, scoped to accessible accounts |
| `get_recurring_transactions` | List detected and manual recurring transactions with per-currency totals; supports status filter and an upcoming-within-days window |

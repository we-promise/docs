...
The assistant processes each chat turn through a tool-call loop:

- The default cap is **8 tool-call rounds** per turn (overridable with `ASSISTANT_MAX_TOOL_CALL_ITERATIONS`).
- On the final permitted round, the assistant sends no tools so the model must answer in prose with whatever it has gathered. This prevents a hard error banner when the cap is reached.
- Tool failures return a structured `{error, hint}` result to the model so it can retry with corrected arguments rather than aborting the turn.

See [Chat response timeout](/llm-support#chat-response-timeout) for how to size `AI_RESPONSE_TIMEOUT` against the round cap.

...
Sure includes an MCP server that lets compatible AI assistants read your financial data and take actions on your behalf.

<Warning>
An authorized client can read and act on the user and family financial data exposed by Sure's MCP tools. Write-capable tools include `import_bank_statement`, `create_goal`, `create_tag`, `update_tag`, `create_category`, `update_category`, `update_transaction`, and `update_budget`. Only connect clients and AI providers you trust, and review tool requests before approving them.
</Warning>
...
This is the best option for Claude.ai and other MCP clients that support OAuth. Sure exposes standard discovery endpoints that let compatible clients register automatically, redirect you to Sure for sign-in, and receive a bearer token with the `read_write` scope. You do not need to register the client or create a token manually.

### Static bearer token

This is the simpler fallback for custom agents, scripts, and deployments where you want to pin the MCP server to a specific Sure user. Set `MCP_API_TOKEN` and `MCP_USER_EMAIL` as environment variables. See [Legacy environment-token fallback](#legacy-environment-token-fallback) for details.
...
3. Open Claude.ai and go to **Settings > Integrations**.
4. Select **Add integration**.
5. Paste the MCP server URL, then select **Add**.
6. Select **Connect**. Your browser redirects to Sure. Sign in if needed, then authorize access.

After authorization, the client can use the Sure tools exposed to it.

### Other MCP-compatible clients

Other MCP-compatible clients follow the same general process: give the client the URL from **Settings > MCP Server**, then complete Sure's sign-in and authorization flow in your browser. The client handles the MCP and OAuth protocol details; you do not need to register it or create an access token manually.

If your client does not support OAuth, use the static bearer token fallback described in [Legacy environment-token fallback](#legacy-environment-token-fallback).
...
Clients should use `tools/list` as the source of truth for tool availability and schemas, as these can vary by Sure version. The following table reflects the current base set.
...
`import_bank_statement` requires a bank-statement PDF already uploaded to Sure and creates an import for review; it does not publish the import automatically. Because MCP access is scoped to the Sure user who authorizes the client, tools can access that user's family data. Review tool calls whenever your client offers that option.
...
<Warning>
The static-token path does not check whether the configured user is active. To revoke MCP access for a static-token deployment, clear or rotate `MCP_API_TOKEN` rather than deactivating the user.
</Warning>

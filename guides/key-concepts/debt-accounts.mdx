...
For fixed-rate loans, Sure generates a full amortization schedule — a payment-by-payment breakdown of how your loan pays off over its entire term. The schedule uses a constant-payment (French) method: interest is charged on the outstanding principal each month, and the remainder of each payment reduces principal. This matches the table your lender uses.

To view the schedule, open a fixed-rate loan account and select the **Schedule** tab.

The tab shows four summary figures at the top:

| Figure | Description |
| --- | --- |
| Monthly payment | The level amount due each period |
...
> [!NOTE]
> The Schedule tab is only available for fixed-rate loans. Variable-rate and adjustable-rate loans are excluded because a schedule built off today's rate would not reflect future payment changes.

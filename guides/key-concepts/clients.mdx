...
The app supports servers hosted under a sub-path. If your self-hosted instance is mounted behind a reverse proxy at a path prefix — for example `https://home.example.com/sure` — enter that full URL when adding the server. The app probes the health endpoint starting at the address you entered and walks up to the origin to find the actual mount point. You can also paste a deep link (such as a session URL copied from your browser) and the app will resolve the correct base automatically.

**Downloads.** Files your Sure server sends as downloads — CSV exports, statement PDFs, transaction attachments, and the import sample CSV — are saved to your macOS Downloads folder. When a download finishes, Sure shows a confirmation toast in the app window. A native macOS notification is also sent when the window is not in front, and replaces the toast when the window is hidden or the server is too old to provide it. Enable Sure notifications in System Settings to receive them. Repeated downloads of the same filename get a numbered suffix.

**Printable reports.** Report pages (URLs ending in `/reports/print`) open in a separate Sure window that carries your session over. Once the page has loaded, the app opens the native macOS print dialog automatically. The print dialog appears once per report window.

**External links.** Links that target a new window follow the same rules as the web app. Pages hosted on a saved server open in a new Sure window. Other `http` and `https` links open in your default browser. Links with non-web schemes or embedded credentials are ignored.

...
The trade detail header reflects the activity label recorded against the trade. If a label is present and Sure has a name for it, the header shows that label (for example, "Transfer" or "Reinvestment"). If the trade has no label, or carries a label Sure does not name, the header falls back to "Buy" or "Sell" based on the transaction amount's sign.
...
A trade labeled **Transfer** is not an acquisition. Sure cannot know what a transferred asset originally cost — the purchase happened in an account that was never tracked here. Treating the transfer date as a cost would produce a figure that looks calculated but is wrong.

Because of this, if any trade in a position is labeled Transfer, the entire position's cost basis is marked unknown. Averaging only the non-transfer purchases and applying that to every unit would be the same fabrication in a quieter form.

The trend indicator for that holding will not display until you enter a manual cost basis. Balances and current value are not affected — they come from the holdings data, not from trade history.

If you know what the asset cost you before the transfer, set a manual cost basis on the holding. Manual and provider-supplied values are preserved and are not cleared by the transfer guard.

...
Dividends and interest are recorded as **trades** (not regular transactions) to maintain data integrity:

- **Dividends** require a security to be selected (the stock/fund paying the dividend)
- **Interest** can optionally specify a security, or will use a synthetic cash security for the account
- Both are recorded with `qty: 0` and `price: 0` to preserve the `security_id NOT NULL` constraint on trades
- This approach matches how brokerages handle investment income internally

When creating a dividend or interest entry:
1. Select the transaction type (Dividend or Interest)
2. For dividends, select the security paying the dividend (required)
3. For interest, optionally select a security (defaults to account cash if none selected)
4. Enter the amount received
5. The entry is saved as a trade with the appropriate label

## Recording Trades

When recording buy or sell trades in investment accounts, you can specify:

- **Security** - The stock, ETF, or other security being traded
- **Quantity** - Number of shares or units
- **Price** - Price per share
- **Fee** (optional) - Transaction fees charged by your broker

### Trade Fees

Transaction fees are included in the total amount calculation:

```
Total Amount = (Quantity × Price) + Fee
```

For example:
- Buy 10 shares at $100/share with a $5 fee = $1,005 total
- Sell 10 shares at $100/share with a $5 fee = $995 total (you receive $995)

Fees are optional and default to $0 if not specified. The fee field is available on both the create and edit forms for trades.
...
| **Dividend** | Dividend payments received (recorded as trades with qty: 0, price: 0) |
| **Interest** | Interest earned (recorded as trades with qty: 0, price: 0) |

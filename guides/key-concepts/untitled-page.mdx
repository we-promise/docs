...
When adding a property, you can look up the estimated market value automatically using a valuation provider (RentCast or Realie). The provider returns the property details and an estimated value which you review before the account is created. Sure then refreshes the value once per day. See [property valuations](/providers/property-valuations) for setup and configuration details.

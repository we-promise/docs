...
When two account syncs run at the same time, a race condition could previously cause transfer matching to stop processing remaining candidates after a conflict. This is now resolved — each candidate is matched in its own isolated savepoint, so a conflict on one pair no longer affects the others.

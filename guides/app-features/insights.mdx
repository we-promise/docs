...
| `maintained_goal_depleted` | A reserve goal has fallen below its target level |
...
## Sure Insights app (experimental)

The Sure Insights app is a native Swift iOS app that receives push notifications when new insights are generated. Notifications respect your locale and honor per-insight notification preferences.

To enable push notifications, register the device token using the Push Subscriptions API (`POST /api/v1/push_subscriptions`). The app sends the APNs token at launch and removes it on sign-out.

Sure Insights is experimental. Features and behavior may change.

...
<Note>
The date format should match your import settings. Common formats include MM/DD/YYYY, DD/MM/YYYY, and YYYY-MM-DD. CSV imports also support the two-digit-year format `DD/MM/YY` (for example, `15/01/24`). Because two-digit years are ambiguous — Ruby interprets `%y` as 1969–2068 — this format is available for CSV imports only, where you can verify the parsed preview against your own data before publishing. It is not available as a global date preference.
</Note>

...
<Note>
The date format should match your import settings. Common formats include MM/DD/YYYY, DD/MM/YYYY, YYYY-MM-DD, and DD/MM/YY. The two-digit year format (DD/MM/YY) is available for CSV imports only; Ruby interprets two-digit years as 1969–2068, so verify the parsed preview matches your data before publishing.
</Note>
...
If you are using the `DD/MM/YY` format, note that two-digit years are interpreted as 1969–2068. Check the parsed preview to confirm dates are read correctly before publishing the import.

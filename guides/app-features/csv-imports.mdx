...
### Sure NDJSON import fails on valid data

When importing a full `all.ndjson` export, preflight previously rejected files that contained rules with a null name or rejected transfers whose referenced transaction had been deleted. Both cases are now handled gracefully:

- **Rules with a null name** pass preflight and import correctly.
- **Orphaned rejected transfers** (where the referenced transaction no longer exists) are skipped and counted rather than blocking the import. A warning is included in the preflight response.

...
<Note>
The `DD/MM/YY` date format (two-digit year) is supported for CSV imports. Because two-digit years are ambiguous, this format is only available in CSV imports where you can verify the parsed preview against your data before confirming. It is not available as a global date preference. Ruby interprets `%y` as years 1969–2068.
</Note>

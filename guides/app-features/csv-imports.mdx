...
### DD/MM/YY dates not recognized

`DD/MM/YY` (two-digit year) is supported for CSV transaction and account balance imports. Select it from the date format picker during import configuration. Because two-digit years are ambiguous — Ruby interprets `%y` as 1969–2068 — this format is only available in CSV imports where you can verify the parsed preview against your own data. It is not available as a global date preference in Settings.

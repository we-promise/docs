...
<Note>
The date format should match your import settings. Common formats include MM/DD/YYYY, DD/MM/YYYY, YYYY-MM-DD, and DD/MM/YY. The DD/MM/YY format (two-digit year) is available for CSV imports only; Sure interprets two-digit years in the range 1969–2068. Always verify the parsed preview against your source data when using this format.
</Note>
...
If your CSV uses two-digit years (for example `15/01/24`), select the **DD/MM/YY** format. Sure interprets two-digit years as falling between 1969 and 2068, so verify the parsed preview before publishing the import.

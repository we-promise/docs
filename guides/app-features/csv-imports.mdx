...
<Note>
The date format should match your import settings. Common formats include MM/DD/YYYY, DD/MM/YYYY, DD/MM/YY, and YYYY-MM-DD.
</Note>
...
Sure supports `DD/MM/YY` (two-digit year) as a CSV-only date format. This format is available in the import date format selector but is intentionally not available as a global date preference, because two-digit years are ambiguous (years are interpreted as 1969–2068). Always verify the parsed date preview in the import review step when using this format.

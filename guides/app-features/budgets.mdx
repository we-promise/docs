...
The **Reports** section gives you a breakdown of income and expenses by category over a selected time period.

Category rows in the income and expense breakdowns are clickable. Clicking a category takes you directly to a filtered transaction list showing only the transactions that make up that category's total. This makes it easy to investigate what's driving a particular line item.

Investment-only rows (such as "Other Investments") are not clickable because those entries are trades rather than regular transactions and cannot be shown in the transaction list.

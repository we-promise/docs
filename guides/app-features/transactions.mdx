...
You can also mark a transaction as excluded. Excluded transactions are removed from budgeting calculations and reports, which is useful for activity you do not want counted in normal spending analysis. Excluded transactions remain visible in the account activity feed, displayed in a greyed-out style, so you can still find and re-include them at any time using the exclude toggle in the transaction drawer.

...
You can also mark a transaction as excluded. Excluded transactions are removed from budgeting calculations and reports, which is useful for activity you do not want counted in normal spending analysis. Excluded entries remain visible in the account activity feed (shown greyed-out) so you can still review them or toggle exclusion back off. Only split parent entries are hidden from the list.
...
You can also create a new category inline while filling out the transaction form — type a name in the category field and select **Create category** to add it without leaving the form.

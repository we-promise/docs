...
You can also mark a transaction as excluded. Excluded transactions are removed from budgeting calculations and reports, which is useful for activity you do not want counted in normal spending analysis. Excluded transactions remain visible in the account activity feed so you can review or re-include them at any time.
...
Rules let you automatically apply categories, merchants, tags, and other attributes to transactions that match a set of conditions. Manage rules from **Settings → Rules**.

### Tag conditions

Rules can now match transactions by their existing tags. Use the **Transaction tag** condition with either of these operators:

- **Equal to**: matches transactions that have a specific tag applied
- **Is empty**: matches transactions that have no tags at all

This lets you build rules that act on already-tagged transactions — for example, applying a category to everything tagged **Reimbursable**, or flagging untagged transactions for review.

You can combine tag conditions with other conditions (name, category, merchant, amount) using AND/OR logic. When you use multiple tag conditions joined by AND, each condition is evaluated independently, so a transaction can satisfy both by having both tags.

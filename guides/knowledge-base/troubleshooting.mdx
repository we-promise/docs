...
<AccordionGroup>
  ...
  <Accordion title="How do I force Sure to re-process transactions?">
    ...
    **Note:** This will incur API costs if using a cloud provider.
    ...
    The reset runs in the background. You can track its progress in **Settings → Debug** under the `ai_cache_reset` category, which logs when the job is enqueued, when it starts, how many cache entries were removed per scope, and any errors encountered.

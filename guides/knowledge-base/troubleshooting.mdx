...
<AccordionGroup>
  ...
  <Accordion title="How do I debug a rule that silently failed?">
    Failures during async rule runs (background jobs) are written to the debug log. Go to **Settings → Debug** to see error entries for any rule that raised an exception during a sync. Auto-categorize provider failures are also propagated there.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="How do I check if the AI cache reset ran?">

    The cache reset runs as a background job. You can verify it ran and see a breakdown of what was cleared in **Settings → Debug** by filtering for the `ai_cache_reset` category.
    ...
    - `warn` entries (up to 5 per scope) for individual records that could not be cleared
    - An `error` entry if a scope fails entirely or the job could not be enqueued
    - An `info` completion entry with the number of AI cache entries removed, broken down by scope

    If you see an enqueue entry but no start entry, the job never reached a worker — check that your worker container is running.

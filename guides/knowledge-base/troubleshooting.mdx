...
When an async rule run (triggered by a sync or background job) fails, the error is not surfaced in the main UI. Failures are written to the debug log so you can diagnose them without tailing worker logs.

<AccordionGroup>
  <Accordion title="How to find rule run failures">
    Go to **Settings → Debug** and look for entries in the `rules` category. Each failed async run logs the error message so you can identify whether the failure is a provider error, a data issue, or a bug in a custom rule.

    Auto-categorize provider failures are also propagated to the debug log, so a rule that fails because the AI provider is unavailable or returns an error will appear there as well.
  </Accordion>
</AccordionGroup>

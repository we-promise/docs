...
The Reports page shows a breakdown of your income and expenses by category for any date range.

### Drilling into categories

Income and expense category rows in the breakdown are clickable. Clicking a category row opens your transaction list pre-filtered to that category and date range, so you can see exactly which transactions make up the total.

Trade-only rows (such as **Other Investments**) are not clickable because those entries cannot be shown in the standard transaction list.

...
### Drilling down from income and expense categories

Category rows in the income and expense breakdown on the Reports page are clickable. Clicking a category row opens your transaction list pre-filtered to that category, date range, and account selection — the same drill-down behavior as the dashboard's Money In / Out widget.

Rows that represent trade-only activity (such as **Other Investments**) are not clickable because the transaction list cannot display trade entries.

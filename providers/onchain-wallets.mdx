...
- The **transfers** in and out, recorded as investment trades labeled "Transfer" when the price for that day is known. Each entry is named "Received" or "Sent" (or the equivalent in your family’s language) followed by the quantity and asset symbol. Transfer trades carry a cash amount of zero — the value lives in quantity and price for cost basis purposes, but no cash leg is recorded (a self-custody wallet has no cash side). When that day’s price is not known, the transfer still appears as an excluded, zero-amount entry — and it is upgraded to a trade on the first sync after the price becomes available.
...
### Limiting transfer history by date

Under **Settings → Providers → On-chain wallets → Advanced**, the **Ignore transfers before** field accepts a date. Transfers recorded before that date are skipped during every sync. Leave the field empty to import everything the explorer reports.

This setting is useful when a wallet has a very long history that you do not need, or when you want to start fresh from a known date without deleting existing accounts.
...
**Transfers appear with a value of 0 and are excluded from totals.** No price was available for that date yet. Once market data covers the range, the next sync upgrades those entries to trades automatically. If they stay at zero, the date is outside what your market data provider can serve. Note that transfer trades always carry a cash amount of zero by design — the value lives in quantity and price, not in a cash figure.

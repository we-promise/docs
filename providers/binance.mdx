...
1. Navigate to **Settings** > **Bank sync** in Sure
2. Click **Connect** on the **Binance** card
3. Enter your API credentials:
   - **Name**: A label for this connection (e.g., "My Binance")
   - **API Key**: Your Binance API key
   - **API Secret**: Your Binance API secret
4. Optionally set a **Sync start date** to limit how far back trade history is fetched. The date must be in the past.
5. Click **Connect**
...
When Binance reports that an asset is no longer in your wallet, Sure removes it from today's holdings so your portfolio stays accurate. Assets are removed based on what the latest sync reports, not on individual price lookups — a temporary pricing outage does not cause a holding to disappear.

If one of the Binance sources (spot, margin, or earn) is temporarily unavailable during a sync, Sure carries forward the last known positions for that source rather than treating the silence as an empty wallet. The held positions are refreshed as soon as that source responds again.
...
1. Navigate to **Settings** > **Bank sync**
...
3. Verify the account type (spot, margin, earn) is supported
4. Verify the account is linked in Sure

If a source such as margin or Earn is temporarily unavailable, Sure preserves the last known positions for that source and logs the issue to **Settings** > **Debug** against the connection. Check there for details on any sync warnings.
...
3. Ensure the sync start date is set to a past date that falls before the trade date

### Stale holdings after selling

If a sold asset continues to appear in your portfolio, manually trigger a sync. Sure removes holdings that Binance no longer reports as part of the wallet during each sync.

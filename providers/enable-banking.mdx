...
Enable Banking API sometimes returns the same logical transaction multiple times with different `entry_reference` values. Sure automatically detects and removes these duplicates using content-based deduplication:
...
This ensures that API-level duplicates are filtered out while preserving legitimately distinct transactions (e.g., two identical payments to the same merchant on the same day with different transaction IDs).

> [!NOTE]
> When `transaction_id` is null for multiple transactions, pure content comparison applies. In rare cases, this could theoretically collapse two genuinely distinct transactions with identical fields, but this trade-off prevents the more common issue of duplicate entries from the API.

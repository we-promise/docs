...
| **Mansa API** | Yes | 100 requests/day | NGX (Nigerian Exchange) securities |
...
#### Warsaw Exchange (GPW) securities

Securities listed on the Warsaw Stock Exchange (XWAR/GPW) are priced in PLN. If you added Polish securities before this fix was released, their historical prices may have been stored as USD. Sure automatically corrects mis-tagged Warsaw prices on upgrade: USD prices are relabeled to PLN, duplicates are removed, and any affected accounts are queued for a sync so holdings revalue at the correct exchange rate.
...
Mansa API provides current quote data for securities listed on the Nigerian Exchange (NGX). It is the only Sure provider with verified NGX coverage.

- **Rate limits**: 100 API requests per day (free tier)
- **Coverage**: NGX (Nigerian Exchange) only
- **API key**: Required — get one at [mansaapi.com](https://mansaapi.com)
- **History**: The free tier returns today's quote only. Historical OHLCV data requires a paid Mansa Pro plan.

<Note>
Because the free tier has no historical endpoint, NGX holdings sync with today's price rather than building full price history. A date range that doesn't include today will return an error rather than empty data.
</Note>

See the [Mansa API provider page](/providers/mansa) for full setup instructions.

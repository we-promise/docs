...
- No foreign exchange metadata: monobank reports the operation amount but not the operation currency, so the raw amount is recorded without FX detail
...
**Sync errors**: Provider sync failures and notes are captured in Sure's debug log. Super admins can find them at **Settings > Debug**, filtered by the `monobank` provider key.

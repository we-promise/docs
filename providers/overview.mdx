...
**Up** is a direct integration for Up bank customers. Sure maps Up's category tags to your Sure categories automatically on import, so transactions you have already categorised in the Up app arrive in Sure with the correct category applied. Categories you have manually set or locked in Sure are preserved on re-sync — the auto-mapping only fills in categories that have not been set yet.

<Note>
Up's categories that have no close Sure equivalent (such as Booze, Pets, and Life Admin) are intentionally left uncategorised so you can apply your own rules or let AI categorise them.
</Note>

...
SCA keypair generation requires Active Record encryption to be configured on your Sure instance. On cloud-hosted Sure this is always the case. On a self-hosted instance, make sure `ACTIVE_RECORD_ENCRYPTION_PRIMARY_KEY`, `ACTIVE_RECORD_ENCRYPTION_DETERMINISTIC_KEY`, and `ACTIVE_RECORD_ENCRYPTION_KEY_DERIVATION_SALT` are set (or that `SECRET_KEY_BASE` is present, from which Sure derives the encryption keys). Without encryption, the private key cannot be stored and the generate action will fail.
...
**Keypair generation fails with an encryption error**: Active Record encryption is not configured on your self-hosted instance. Set `ACTIVE_RECORD_ENCRYPTION_PRIMARY_KEY`, `ACTIVE_RECORD_ENCRYPTION_DETERMINISTIC_KEY`, and `ACTIVE_RECORD_ENCRYPTION_KEY_DERIVATION_SALT` in your environment (or ensure `SECRET_KEY_BASE` is set), then try again. See the [self-hosting encryption setup](/self-hosting#turning-on-active-record-encryption-after-first-boot) guide for details.

...
When you trigger a manual sync on a Plaid-connected account, Sure requests a fresh data pull from the institution before reading new transactions. This means manual syncs fetch the latest data directly rather than waiting for a Plaid webhook.

After sending the refresh request, Sure polls Plaid for the updated transaction cursor and syncs once the new data arrives. If no new transactions appear within the polling window (roughly 3 minutes), Sure falls back to a normal sync with whatever data Plaid currently has.

To prevent duplicate refresh calls, refreshes are coalesced: if you trigger another sync within 5 minutes of an already-in-progress refresh, the second request is dropped and the first finishes normally.

### Cache store requirement for self-hosted instances

The coalescing logic requires a shared cache store. Sure skips the Plaid refresh request (and logs a warning) if the cache store is the default file-based or in-memory store. To enable manual refresh on a self-hosted instance, configure one of the following:
...
Without a shared cache, manual syncs still work but do not trigger a fresh pull from Plaid before fetching transactions.

...
CoinStats is a third-party provider that allows you to sync cryptocurrency holdings into Sure. It supports wallet balances, DeFi positions (staking, liquidity pools, yield farming), and centralized exchange accounts.

## How it works

CoinStats connects to your crypto assets in three ways:

- **Wallets** — Track token balances across multiple blockchain networks by wallet address
- **DeFi positions** — Automatically discover staking, LP, and yield farming positions associated with your wallets
- **Exchange accounts** — Connect centralized exchanges (e.g. Binance, Kraken) through CoinStats portfolio API

When you add a connection, Sure syncs balances, transaction history, and real-time price data.
...
- `ETH (Aave Staking)` — a staked ETH position on Aave
- `USDC (Uniswap Rewards)` — reward tokens from a Uniswap LP
...
You can connect a centralized exchange account through CoinStats to sync your portfolio.

1. Select **Link exchange** from the CoinStats connection page
2. Choose your exchange from the supported list
3. Enter the required credentials (API key, secret, and any additional fields specific to the exchange)
4. Sure creates a portfolio on CoinStats and syncs your exchange holdings

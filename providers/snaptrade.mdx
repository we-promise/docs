...
### Holdings are missing after a successful sync

Some SnapTrade apps registered after SnapTrade's 2026 API cutoff receive an HTTP 410 Gone response from the legacy `/positions` endpoint. Sure handles this gracefully and the sync still reports success, but no holdings are imported.

Sure uses the `/positions/all` endpoint to avoid this issue. If you connected a brokerage before this fix was applied and your holdings are still empty, disconnect and reconnect the brokerage to trigger a fresh sync against the updated endpoint.

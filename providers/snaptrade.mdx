...
### Holdings are missing after connecting a brokerage

SnapTrade deprecated the `/positions` endpoint for apps registered after their 2026 cutoff. If your brokerage account shows a balance but no holdings, your Sure instance may be running a version that still calls the old endpoint. Upgrade to the latest Sure release, which uses the `/positions/all` replacement endpoint.

After upgrading, trigger a manual sync from the SnapTrade account card to import your holdings.

...
### Account balance syncs but holdings are empty

SnapTrade deprecated the `/positions`, `/holdings`, and `/options` endpoints for apps registered after their 2026 cutoff. Affected apps receive an HTTP 410 Gone response, which Sure previously treated as an empty portfolio rather than an error — so the sync appeared to succeed while no holdings were imported.

Sure now uses the `/positions/all` replacement endpoint. If you connected your brokerage before this fix was deployed and your holdings are still missing, disconnect and reconnect the brokerage to trigger a fresh sync.

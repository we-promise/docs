...
The device code has a limited validity window. If Sure shows an error saying the code has expired, start a new connection attempt to receive a fresh code. Errors during polling that say "not confirmed yet" retry automatically without needing a restart.

...
### Holdings are empty after connecting a brokerage

If your account shows a balance but no holdings after syncing, your SnapTrade app may have been registered after SnapTrade's 2026 API cutoff. Older `/positions` and `/holdings` endpoints return HTTP 410 Gone for apps registered after that date, which caused Sure to silently import balances without any positions.

Sure now uses the `/positions/all` endpoint, which is the documented replacement. If you were affected, run a manual sync from the account card to re-import your holdings.

...
### Holdings not appearing on newly connected accounts

SnapTrade deprecated the `/positions`, `/holdings`, and `/options` endpoints for apps registered after their 2026 cutoff, returning HTTP 410 Gone. Sure now uses the `/positions/all` replacement endpoint, so newly connected accounts import holdings correctly.

If you connected a brokerage before this fix and see a balance but no holdings, run a manual sync from the account card to pull positions using the updated endpoint.

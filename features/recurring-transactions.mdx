...
You can mark multiple transactions to the same payee as recurring even if they have different amounts — for example, two separate subscriptions to the same merchant at different price points. Each distinct amount creates its own recurring entry. An exact duplicate (same account, payee, currency, and amount) is still blocked to prevent accidental double-entries.
...
### Amount variance banding

When building or updating a recurring entry's expected amount range, the system only includes historical transactions whose amount is within 2× of the anchor amount (double or half). This prevents unrelated charges to the same merchant — for example, a $3 fee and a $19 subscription — from being blended into a single recurring entry with a fabricated average amount.

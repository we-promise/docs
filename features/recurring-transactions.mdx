...
Two manual recurring transactions to the same payee are allowed as long as their amounts differ (for example, a $3.00 fee and a $19.68 charge from the same merchant). An exact duplicate — same account, payee, currency, and amount — is still blocked.
...
- Variance band matching uses a 2× ratio threshold: a historical transaction is only considered the same recurring charge if its amount is within double or half of the target amount. This prevents unrelated charges to the same merchant from being blended into a single recurring record.

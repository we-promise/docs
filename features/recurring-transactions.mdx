...
Two transactions to the same payee are only considered duplicates when they share the same account, payee, currency, **and amount**. This means you can mark two payments to the same merchant at different amounts as separate recurring transactions — for example, a $3.00 fee and a $19.68 subscription from the same provider will each get their own recurring entry.

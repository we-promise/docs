...
Two manual recurring transactions for the same payee are allowed as long as their amounts differ — for example, a $3.00 fee and a $19.68 charge from the same merchant are tracked as separate recurring entries. An exact duplicate (same account, payee, currency, and amount) is still blocked.
...
### Amount variance band

When the system calculates the expected amount range for a recurring transaction, it only includes historical entries whose amount is within 2× of the target amount (i.e., between half and double the target). This prevents unrelated charges from the same merchant — such as a small fee and a larger monthly payment — from being blended into a single projected amount.
...
- Projected amounts are calculated only from transactions within 2× of the target amount, so distinct charges from the same merchant are not blended together

...
Two manual recurring transactions to the same payee are allowed as long as their amounts differ (for example, a $3.00 fee and a $19.68 subscription from the same merchant). Marking a transaction as recurring is blocked only when an identical entry — same account, payee, currency, **and** amount — already exists.
...
When building a recurring transaction's expected amount range, Sure only includes historical entries whose amount is within 2× of the target amount (i.e., no more than double or half the anchor charge). This prevents unrelated charges from the same merchant — such as a small fee and a larger subscription — from being blended into a single projected amount that matches neither transaction.

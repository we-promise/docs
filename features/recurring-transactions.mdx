...
If a manual recurring transaction already exists for the same pattern (same account, merchant, amount, and currency), the **Mark as Recurring** button is disabled and shows a message explaining why. You can manage the existing recurring transaction from **Settings → Recurring**.

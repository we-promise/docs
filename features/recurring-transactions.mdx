...
Click **Clean Up Stale** to mark recurring transactions as inactive if they haven't occurred recently. Automatically detected recurring transactions use a 2-month threshold; manually marked recurring transactions use a 6-month threshold. Recurring transfers are matched by their transfer pair (an outflow on the source account paired with an inflow on the destination account) rather than by name, so transfers stay active as long as the pair keeps occurring and are retired only when the pair stops.
...
When building or refreshing a manual recurring entry's expected amount range, the system only includes historical transactions whose amount is within 2× of the anchor amount (double or half). This prevents unrelated charges from the same merchant — for example, a small fee and a larger monthly due — from being blended into a single projected amount that matches neither real transaction.

### Cleanup logic

Recurring transactions are automatically marked inactive when:

- No matching transaction has occurred within the staleness threshold (2 months for automatically detected patterns, 6 months for manually marked patterns)

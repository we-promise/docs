...
If a manual recurring transaction already exists for the same pattern (same account, merchant, amount, and currency), the **Mark as Recurring** button is disabled and shows the reason inline. You do not need to attempt the action to discover the conflict.

...
Two manual recurring transactions for the same payee are allowed as long as their amounts differ — for example, a $3.00 fee and a $19.68 charge from the same merchant can each have their own recurring record. An exact duplicate (same account, payee, currency, and amount) is still blocked.
...
- Derivative instruments (options, etc.) from brokerage accounts are excluded from recurring detection

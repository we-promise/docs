...
Two manual recurring transactions to the same payee are allowed as long as their amounts differ — for example, a $3.00 fee and a $19.68 charge from the same merchant are tracked as separate recurring entries. An exact duplicate (same account, payee, currency, and amount) is still blocked.
...
When building the expected amount range for a manual recurring transaction, Sure only includes historical entries whose amount is within 2× of the target amount (i.e., between half and double the target). This prevents unrelated charges from the same merchant — such as a small fee alongside a larger monthly payment — from being blended into a single projected amount.

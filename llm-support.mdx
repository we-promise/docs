...
Sure can call the Anthropic Messages API directly instead of going through an OpenAI-compatible proxy. The native integration covers chat, auto-categorization, merchant detection, and provider-merchant enhancement, and records per-call token usage (including prompt-cache reads and writes) in the LLM cost ledger.

Set `LLM_PROVIDER=anthropic` to make Sure route AI traffic through Anthropic:
...
`ANTHROPIC_API_KEY` is accepted as an alias for `ANTHROPIC_ACCESS_TOKEN`.

### Supported models

Any model whose ID starts with `claude` is accepted. Pricing is tracked in the LLM cost ledger for the following models (input / output, per million tokens):
...
Models not listed above still work — they just won't have a cost estimate attached to their usage records.

### Custom Anthropic-compatible endpoints

To route Anthropic calls through Amazon Bedrock, Google Vertex AI, or another Anthropic-compatible proxy, configure a custom base URL and an explicit model name. Custom endpoints bypass the `claude` prefix check, so any model ID is accepted.

### Environment variables

| Variable | Description | Default |
| --- | --- | --- |
| `ANTHROPIC_ACCESS_TOKEN` | Anthropic API key | — |
| `ANTHROPIC_API_KEY` | Alias for `ANTHROPIC_ACCESS_TOKEN` | — |
| `ANTHROPIC_MODEL` | Default Claude model | `claude-sonnet-4-6` |
| `ANTHROPIC_REQUEST_TIMEOUT` | HTTP timeout in seconds | `600` |
| `ANTHROPIC_MAX_TOKENS` | Upper bound on output tokens per request | `4096` |

...
| --- | --- | --- | --- |
| `ASSISTANT_TYPE=builtin` (default) | Uses OpenAI provider | Bayes first, then OpenAI | Uses OpenAI provider |
| `ASSISTANT_TYPE=external` | Uses external agent | Bayes first, then OpenAI | Uses OpenAI provider |

If you use an external agent for chat, you still need `OPENAI_ACCESS_TOKEN` set for the LLM categorization pass and merchant detection to work. Families with at least 20 categorized transactions across 2 or more categories will see Bayes reduce or eliminate LLM calls for categorization.
...
<Note>
The external assistant handles chat only. Auto-categorization and merchant detection still use the OpenAI-compatible provider (`OPENAI_ACCESS_TOKEN`).
</Note>

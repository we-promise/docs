...
**Auto-categorization and merchant detection (background)** — background jobs that classify transactions and detect merchants. These always use the OpenAI-compatible provider (`OPENAI_ACCESS_TOKEN`), regardless of what the chat assistant uses.
...
If you use an external agent for chat, you still need `OPENAI_ACCESS_TOKEN` set for auto-categorization and merchant detection to work.

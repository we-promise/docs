...
<Note>
When the tool-call round limit is reached, the assistant sends one final request with no tools available so the model can answer in prose with whatever it gathered. This means the conversation always produces a response rather than ending with an error banner.
</Note>

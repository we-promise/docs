...
- **URI Base / Base URL** — Custom endpoint URL (leave blank for the default cloud endpoint)
...
Settings in the UI override environment variables. Environment variables take precedence when both are set.
...
The cache is cleared asynchronously in the background. You can monitor the reset in **Settings → Debug** by filtering for the `ai_cache_reset` category, which logs:

- When the reset is enqueued and when the job starts
- Any individual records or scopes that could not be cleared
- A completion summary with the number of AI cache entries removed, broken down by scope

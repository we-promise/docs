...
<Note>
`AI_RESPONSE_TIMEOUT` must be larger than `OPENAI_REQUEST_TIMEOUT`. The watchdog covers the whole turn, so setting it equal to or below the per-call limit means any turn that uses a tool will always time out.
</Note>

...
The form is organized into the following groups:
...
**Chat**
- **Chat Response Timeout** — Whole-turn watchdog in seconds (default: `90`). See [Chat response timeout](#chat-response-timeout) for sizing guidance.
...
### Monitoring a cache reset

Every stage of the reset job is written to the debug log under the `ai_cache_reset` category. You can filter for it at **Settings → Debug**:

- **info** — logged when the reset is enqueued from the rules page, and again when the job starts on a worker
- **warn** — logged for individual records that could not be cleared (capped at 5 per scope) and when the job receives no family
- **error** — logged when a scope fails outright or when the job could not be enqueued
- **info (completion)** — logged when the job finishes, with a count of AI cache entries removed broken down by scope

If you see an enqueue `info` entry but no matching start entry, the job never reached a worker. If you see a start entry but no completion entry, the job was interrupted mid-run.

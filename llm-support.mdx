...
| `LLM_MAX_RESPONSE_TOKENS` | Tokens reserved for the model's reply. When set, this value is also sent to the provider as `max_tokens` (OpenAI) or `max_output_tokens` (Anthropic Responses API) to cap the model's output length | `512` |
...
# (1 + 2) × 300 = 900s model time + ~300s headroom = 1200s

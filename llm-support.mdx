...
The cache is cleared asynchronously in the background. You can monitor the reset in **Settings → Debug** — filter by the `ai_cache_reset` category to see:

- When the reset was enqueued and when the job started
- How many AI cache entries were removed, broken down by scope
- Any individual records that could not be cleared (logged as warnings)
- Errors if a scope failed outright or the job could not be enqueued
...
services:
  sure:
    environment:
      ...
      - OPENAI_MODEL=llama3.1:13b
      # Tune timeouts and tool-call depth for slow local models
      # - OPENAI_REQUEST_TIMEOUT=300
      # - ASSISTANT_MAX_TOOL_CALL_ITERATIONS=2
      # - AI_RESPONSE_TIMEOUT=1200

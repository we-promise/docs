...
| `LLM_MAX_RESPONSE_TOKENS` | Tokens reserved for the model's reply; sent to the provider as `max_tokens` / `max_output_tokens` when set | `512` |
...
### Tool-round cap and grace turn

When the assistant reaches the tool-call limit, it is given one final round with no tools available. This forces the model to answer in prose using whatever it gathered, rather than returning an error banner. The hard limit remains as a backstop if the model still does not produce a text response.

...
If you plan to use the bundled PostgreSQL or Redis subcharts, add their repositories first:
...
Download the latest Helm chart from the [Sure releases page](https://github.com/we-promise/sure/releases):

```bash
# Download the latest chart archive
curl -LO https://github.com/we-promise/sure/releases/latest/download/sure-helm-chart.tgz

# Extract the chart
tar -xzf sure-helm-chart.tgz
```

Alternatively, you can clone the repository and use the chart directly:
...
helm upgrade sure charts/sure -n sure \
